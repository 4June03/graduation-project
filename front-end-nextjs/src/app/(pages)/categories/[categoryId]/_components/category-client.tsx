...
import type { CategoryMotorbike } from "../_lib/api"
...
export function CategoryClient({ categoryId, categoryName, motorbikes, categories }: CategoryClientProps) {
  ...
  const filteredProducts = motorbikes.filter((motorbike) => {
    ...
    // Filter by brand
    if (selectedBrands.length > 0 && !selectedBrands.includes(motorbike.brand.toLowerCase())) {
      return false
    }
  ...
  const isFiltersDefault =
    priceRange[0] === 0 && priceRange[1] === 200000000 && selectedBrands.length === 0 && selectedColors.length === 0

  // Convert motorbikes to ProductData format for ProductGrid
  const productsForGrid = currentProducts.map((motorbike) => ({
    id: motorbike.id,
    name: motorbike.name,
    image: motorbike.image,
    price: motorbike.price,
    oldPrice: motorbike.oldPrice,
    brand: motorbike.brand,
    isNew: motorbike.isNew,
  }))
      ...
      <div className="container py-8">
        <Tabs defaultValue={categoryId} className="w-full">
          <div className="flex justify-between items-center mb-6">
            <CategoryTabs categories={categories} currentCategoryId={categoryId} />
